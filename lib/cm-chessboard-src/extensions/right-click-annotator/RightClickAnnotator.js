...
export class RightClickAnnotator extends Extension {
    ...
    isEnabled() {
        return localStorage.getItem("chessengineered_training_arrows") !== "off"
    }
    ...
    onMouseDown(event) {
        // left click: clear arrows and markers
        if (event.button === 0) {
            this.chessboard.removeArrows()
        }
        // right button only
        if (event.button !== 2 || !this.isEnabled()) {
            return
        }
    ...
    onMouseUp(event) {
        ...
        if (!start || event.button !== 2 || !this.isEnabled()) {
            return
        }
    ...
    findSquareFromEvent(event) {
        ...
        if (el) {
            return el.getAttribute("data-square")
        }
        // Fallback: compute square from mouse coordinates
        return this.squareFromPoint(event.clientX, event.clientY)
    }
    ...
    squareFromPoint(clientX, clientY) {
        ...
        const col = Math.floor((x - view.borderSize) / view.squareWidth)
        const row = Math.floor((y - view.borderSize) / view.squareHeight)
        if (col < 0 || col > 7 || row < 0 || row > 7) {
            return undefined
        }
        let file, rank
        if (this.chessboard.state.orientation === "w") {
            file = col
            rank = 7 - row
        } else {
            file = 7 - col
    ...
    onMouseMove(event) {
        if (!this.dragStart || !this.isEnabled()) {
            return
        }
    ...
    modifiersToColorKey(modifiers) {
        if (modifiers.shift && modifiers.alt) return "danger"
        if (modifiers.shift) return "info"
        if (modifiers.alt) return "success"
        return "warning"
    }
